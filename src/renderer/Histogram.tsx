...
interface HistogramProps {
    ...
    setData: React.Dispatch<React.SetStateAction<Array<{ date: string; value: number }>>>;
  }
...
const Histogram: React.FC<HistogramProps> = ({ params, data, setData }) => {
  ...
  const fetchData = async () => {
    try {
        ...
        const response = await fetch(`https://data.nasdaq.com/api/v3/datasets/RATEINF/CPI_USA.csv${queryString}&api_key=GjnzxkK7m5ybzh12gzx7`)
        const responseStr = await response.text()
        const responseData = responseStr.split('\n').slice(1).map((el) => {
            const arr = el.split(',')
            const obj = {
                "date": arr[0], 
